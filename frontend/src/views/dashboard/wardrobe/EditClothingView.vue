<template>
  ...
  <div v-else-if="!item" class="error-container">
    <el-empty description="衣物不存在">
      <el-button @click="goBack">返回</el-button>
    </el-empty>
  </div>
  <div v-else class="edit-clothing-container">
    <!-- Same as AddClothingView but with pre-filled data -->
...
const item = ref<Clothing | null>(null)
...
onMounted(async () => {
  ...
  const clothing = wardrobeStore.getItemById(id)
  ...
  if (clothing) {
    ...
    const fetched = wardrobeStore.getItemById(id)
    if (fetched) {
      item.value = fetched
      Object.assign(form, {
        name: fetched.name,
...
async function handleSave() {
  if (!formRef.value || !item.value) return

  await formRef.value.validate(async (valid) => {
    if (valid) {
      saving.value = true
      try {
        const result = await wardrobeStore.updateItem(item.value.id, {
          name: form.name,
